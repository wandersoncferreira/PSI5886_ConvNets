...
Como podemos perceber, a complexidade da express\~ao relacionando os inputs e outputs aumenta consideravelmente com o aumento de camadas. A determina\c{c}\~ao dos pesos $W$ era um problema em aberto at\'e a d\'ecada de 80, quando foi proposto um algoritmo computacionalmente eficiente para encontra-los: o algoritmo de backpropagation. A implementa\c{c}\~ao realmente eficiente desse algoritmo s\'o foi poss\'ivel recentemente com a evolu\c{c}\~ao de GPUs, cuja capacidade computacional paralela \'e altissima. 

\subsection{Algoritmo de Backpropagation}

O algoritmo de backpropagation \'e um metodo iterativo baseado em derivadas parciais. Explicaremos nessa se\c{c}\~ao o procedimento de maneira simplificada. O algoritmo formula o problema de determina\c{c}\~ao dos pesos como um problema de otimiza\c{c}\~ao, definindo uma vari\'avel de erro $e_{(W_k)}$. Seja $e_{(W_k)}$ uma fun\c{c}\~ao escalar cont\'inua e deriv\'avel dos pesos $W_k$, calculada para todos os exemplos do conjunto de treinamento $C_T$:
